...
from .sgds import SmallObjectGuidedDetailLoss
from .detail_alignment import DetailGateAlignmentLoss
...
class DEIMCriterion(nn.Module):
    ...
    def __init__(self, \
        ...
        sgds_small_area=32 * 32,
        sgds_small_area_ratio=0.0,
        sgds_box_expand=0.25,
        sgds_neg_weight=0.05,
        sgds_dice_weight=0.5,
        sgds_smooth_kernel=3,
        dga_small_area=32 * 32,
        dga_small_area_ratio=0.0,
        dga_box_expand=0.15,
        dga_inner_shrink=0.15,
        dga_target_mode="ring",
        dga_dice_weight=0.1,
        dga_smooth_kernel=3,
        dga_warmup_epoch=4,
        ):     
        ...
        self.epoch = 0
        self.sgds_loss = SmallObjectGuidedDetailLoss(
            small_area=sgds_small_area,
            small_area_ratio=sgds_small_area_ratio,
            box_expand=sgds_box_expand,
            neg_weight=sgds_neg_weight,
            dice_weight=sgds_dice_weight,
            smooth_kernel=sgds_smooth_kernel,
        )
        self.dga_loss = DetailGateAlignmentLoss(
            small_area=dga_small_area,
            small_area_ratio=dga_small_area_ratio,
            box_expand=dga_box_expand,
            inner_shrink=dga_inner_shrink,
            target_mode=dga_target_mode,
            dice_weight=dga_dice_weight,
            smooth_kernel=dga_smooth_kernel,
            warmup_epoch=dga_warmup_epoch,
        )
    ...
    def loss_sgds(self, outputs, targets, indices, num_boxes):
        return {"loss_sgds": self.sgds_loss(outputs.get("sgds_maps", []), targets)}

    def loss_dga(self, outputs, targets, indices, num_boxes):
        return {"loss_dga": self.dga_loss(outputs.get("dga_maps", []), targets, self.epoch)}
