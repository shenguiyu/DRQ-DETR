...
class DFINETransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.sdq_levels = int(sdq_levels)
        self.sdq_pre_topk = int(sdq_pre_topk)
        self.sdq_query_topk = int(sdq_query_topk)
        self.sdq_grid_size = float(sdq_grid_size)
        assert self.sdq_levels >= 0, 'sdq_levels must be non-negative'
        assert self.sdq_pre_topk >= 0, 'sdq_pre_topk must be non-negative'
        assert self.sdq_query_topk >= 0, 'sdq_query_topk must be non-negative'
        assert len(feat_channels) > self.sdq_levels, 'SDQ needs at least one value feature level'

        if self.sdq_levels > 0:
            # Strides may describe all decoder inputs or only value levels.
            if len(feat_strides) == len(feat_channels):
                feat_strides = feat_strides[self.sdq_levels:]
            else:
                assert len(feat_strides) == len(feat_channels) - self.sdq_levels,\
                    'feat_strides must describe all inputs or value levels only'

        value_feat_channels = feat_channels[self.sdq_levels:]
        self.sdq_feat_channels = feat_channels[:self.sdq_levels]
        self.value_feat_channels = value_feat_channels
        
        assert len(value_feat_channels) <= num_levels
        assert len(feat_strides) == len(value_feat_channels)     
    ...
    def _reset_parameters(self, feat_channels):
        ...
        for m, in_channels in zip(getattr(self, 'sdq_input_proj', []), feat_channels[:self.sdq_levels]):
            if in_channels != self.hidden_dim:
                init.xavier_uniform_(m[0].weight)

        value_feat_channels = feat_channels[self.sdq_levels:]
        for m, in_channels in zip(self.input_proj, value_feat_channels):   
            if in_channels != self.hidden_dim:
                init.xavier_uniform_(m[0].weight)  

    def _build_input_proj_layer(self, feat_channels):    
        # SDQ levels provide sparse query candidates only. The remaining levels
        # are the value features used by deformable cross-attention.
        self.sdq_input_proj = nn.ModuleList()
        sdq_feat_channels = feat_channels[:self.sdq_levels]
        value_feat_channels = feat_channels[self.sdq_levels:]

        for in_channels in sdq_feat_channels:
            if in_channels == self.hidden_dim:
                self.sdq_input_proj.append(nn.Identity())
            else:
                self.sdq_input_proj.append(
        ...
        for in_channels in value_feat_channels:   
            if in_channels == self.hidden_dim:
        ...
        in_channels = value_feat_channels[-1]     
        
        for _ in range(self.num_levels - len(value_feat_channels)):   
            if in_channels == self.hidden_dim:     
                self.input_proj.append(nn.Identity())    
            else:
                self.input_proj.append(
    ...
    def _get_encoder_input(self, feats: List[torch.Tensor]): 
        ...
        sdq_flatten, sdq_spatial_shapes = None, None
        if self.sdq_levels > 0:
            sdq_proj_feats = [
                self.sdq_input_proj[i](feat) for i, feat in enumerate(feats[:self.sdq_levels])
            ]
            sdq_flatten, sdq_spatial_shapes = [], []
            for feat in sdq_proj_feats:
                _, _, h, w = feat.shape
                sdq_flatten.append(feat.flatten(2).permute(0, 2, 1))
                sdq_spatial_shapes.append([h, w])
            sdq_flatten = torch.concat(sdq_flatten, 1)
            feats = feats[self.sdq_levels:]
        ...
        return feat_flatten, spatial_shapes, sdq_flatten, sdq_spatial_shapes   
    ...
    def _get_decoder_input(self,   
        ...
        candidate_memory = output_memory
        candidate_logits = enc_outputs_logits
        candidate_anchors = anchors
        sdq_output_memory, sdq_outputs_logits, sdq_candidate_anchors = None, None, None
        if self.sdq_levels > 0 and self.sdq_pre_topk > 0 and sdq_memory is not None:
            sdq_anchors, sdq_valid_mask = self._generate_anchors(
                sdq_spatial_shapes,
                grid_size=self.sdq_grid_size,
                device=memory.device,
            )
            if memory.shape[0] > 1:
                sdq_anchors = sdq_anchors.repeat(memory.shape[0], 1, 1)
                sdq_valid_mask = sdq_valid_mask.repeat(memory.shape[0], 1, 1)

            sdq_memory = sdq_valid_mask.to(sdq_memory.dtype) * sdq_memory
            sdq_k = min(self.sdq_pre_topk, sdq_memory.shape[1])
            sdq_saliency = sdq_memory.detach().square().mean(dim=-1)
            sdq_saliency = sdq_saliency.masked_fill(
                ~sdq_valid_mask.squeeze(-1),
                torch.finfo(sdq_saliency.dtype).min,
            )
            _, sdq_ind = torch.topk(sdq_saliency, sdq_k, dim=-1)
            sdq_memory = sdq_memory.gather(
                dim=1,
                index=sdq_ind.unsqueeze(-1).repeat(1, 1, sdq_memory.shape[-1]),
            )
            sdq_anchors = sdq_anchors.gather(
                dim=1,
                index=sdq_ind.unsqueeze(-1).repeat(1, 1, sdq_anchors.shape[-1]),
            )

            sdq_output_memory = self.enc_output(sdq_memory)
            sdq_outputs_logits = self.enc_score_head(sdq_output_memory)
            sdq_candidate_anchors = sdq_anchors
            candidate_memory = torch.cat([output_memory, sdq_output_memory], dim=1)
            candidate_logits = torch.cat([enc_outputs_logits, sdq_outputs_logits], dim=1)
            candidate_anchors = torch.cat([anchors, sdq_anchors], dim=1)
 
        
        if sdq_output_memory is not None and self.sdq_query_topk > 0:
            sdq_query_k = min(self.sdq_query_topk, self.num_queries, sdq_output_memory.shape[1])
            semantic_query_k = self.num_queries - sdq_query_k
            enc_sem_memory, enc_sem_logits, enc_sem_anchors =\
                self._select_topk(output_memory, enc_outputs_logits, anchors, semantic_query_k)
            enc_sdq_memory, enc_sdq_logits, enc_sdq_anchors =\
                self._select_topk(sdq_output_memory, sdq_outputs_logits, sdq_candidate_anchors, sdq_query_k)
            enc_topk_memory = torch.cat([enc_sdq_memory, enc_sem_memory], dim=1)
            enc_topk_anchors = torch.cat([enc_sdq_anchors, enc_sem_anchors], dim=1)
            enc_topk_logits = torch.cat([enc_sdq_logits, enc_sem_logits], dim=1) if self.training else None
        else:
            enc_topk_memory, enc_topk_logits, enc_topk_anchors =\
                self._select_topk(candidate_memory, candidate_logits, candidate_anchors, self.num_queries)     
        ...
        return content, enc_topk_bbox_unact, enc_topk_bboxes_list, enc_topk_logits_list, candidate_logits     
    ...
    def forward(self, feats, targets=None):  
        ...
        memory, spatial_shapes, sdq_memory, sdq_spatial_shapes = self._get_encoder_input(feats)  
